...
The face matches the reservation map badge (`TripStatsBadge`): a pale white veil, a white hairline, dark type, a soft black shadow. The web pill blurs the map; this one cannot, so the veil is a step more opaque than `bg-white/55`. Android `elevation` stays at 0. The arrival chip is the same row as the badge: a blue glyph, the distance, a dot, the time.
