...
The face is a vertical grey gradient (`fillTop` → `fillBottom`), the notification wash. The contour is a second full-height gradient (`rimLight` → `rimShade`) inset by one point, so only the edge shows — light on top, dark underneath. That is the bevel. It is not a highlight band: `has no highlight band` fails if a gradient stop is placed over the type. Android `elevation` stays at 0.
