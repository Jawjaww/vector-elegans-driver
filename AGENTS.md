...
The face is neutral grey with a barely visible vertical wash (`fillTop` / `fillBottom`). The **contour** is beveled (`rimLight` / `rimShade`, 2 px inset) with corner glares in the top-left and bottom-right only — not a band across the type. `bevels the contour and lights the top and bottom corners only` and `washes the grey face in a barely visible step` pin the split. Android `elevation` stays at 0. Cards use `OVERLAY_CARD_RADIUS` (20) and 16 pt type.
